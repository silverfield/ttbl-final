...
TdUspora3::TdUspora3(vector<string> iArguments) {
	cPreprocessed = false;
...
Ret TdUspora3::fPreprocess(Object *iObject) {
	cGraph = dynamic_cast<TdGraph*>(iObject);

	//----- preprocessing -----
	INFO("Starting preprocessing of USP-OR-A-3 oracle (for TD graphs)" << endl);
	...
	INFO("Obtaining USP trees among ANs" << endl);
	vector<int> lAnVec;
	for (size_t i = 0; i < lAns.size(); i++) {
		lAnVec.push_back(cUgGraph->fGetNodeIndex(lAns[i]));
	}
	cFAnUspTrees = vector<vector<UspTree> >(lAns.size(), vector<UspTree>(lAns.size(), UspTree()));
	lOldPercentage = -1;
	for (int i = 0; i < lAnVec.size(); i++) {
		int lPercentage = (i * 100) / lAnVec.size();
		if (lPercentage != lOldPercentage) {
			INFO("Progress " << lPercentage << " % " << endl);
			lOldPercentage = lPercentage;
		}
		...
		gLogger->fInfoOn(false);
		vector<vector<Path> > *lUsps = fRestrUSP(cGraph, lAnVec[i], lAnVec, NULL, ANY_RANGE);
		gLogger->fInfoOn(true);
		for (int j = 0; j < lAnVec.size(); j++) {
			UspTree *lTree = &(cFAnUspTrees[i][j]);
			for (size_t k = 0; k < (*lUsps)[j].size(); k++) {
				lTree->fAdd((*lUsps)[j][k]);
			}
		}
	...
	INFO("Ending preprocessing of USP-OR-A-3 oracle (for TD graphs)" << endl);
...
int TdUspora3::fGetSpace() {
	...
	lBytes += fSizeOf(cFAnUspTrees);
	for (size_t i = 0; i < cFAnUspTrees.size(); i++) {
		lBytes += fSizeOf(cFAnUspTrees[i]);
		for (size_t j = 0; j < cFAnUspTrees[i].size(); j++) {
			lBytes += cFAnUspTrees[i][j].fGetSpace();
		}
	}
...
Connection TdUspora3::fAnUspTreeConn(int iFrom, int iTime, int iTo) {
	if (iFrom == iTo) {
		Connection lConn;
	...
	return fConnUspTree(iFrom, iTime, iTo, cFAnUspTrees[lAnFrom][lAnTo], cGraph);
}
...
#define TERMINATE if (lRet == OK) {gAlgTimer->fStopTimer();} \
		if (lLocalConn.fIsEmpty()) { \
			...
			INFO("Ending querying USP-OR-A-3 oracle (for TD graphs) for the connections" << endl); \
			return lLocalConn; \
		}

Connection TdUspora3::fQueryConn(string iFrom, int iTime, string iTo) {
	if (cPreprocessed == false) {
		ERR("TdUspora3::fQueryConn: Preprocessing not carried out" << endl);
		return Connection();
	}

	//----- compute -----
	INFO("Starting querying USP-OR-A-3 oracle (for TD graphs) for a connections" << endl);
	Ret lRet = gAlgTimer->fStartNewTimer();
...
//	}
	...
	for (unordered_set<int>::iterator i = cBLans[lToCityIndex].begin();
		...
		for (unordered_set<int>::iterator j = cFLans[lFromCityIndex].begin();
			...
			Connection lUspConn = fAnUspTreeConn(*j, lFLanTime, *i);
...
int TdUspora3::fQueryEa(string iFrom, int iTime, string iTo) {
	INFO("Starting querying USP-OR-A-3 oracle (for TD graphs) for the earliest arrival" << endl);
	Ret lRet = gAlgTimer->fStartNewTimer();

	Connection lConn = fQueryConn(iFrom, iTime, iTo);

	if (lRet == OK) {gAlgTimer->fStopTimer();}
	INFO("Ending querying USP-OR-A-3 oracle (for TD graphs) for the earliest arrival" << endl);

	return lConn.fGetLength();
}

string TdUspora3::fSGetHelp() {
	stringstream lSs (stringstream::out);

	lSs << "HELP USP-OR-A-3 oracle (for time graphs)" << endl <<
			"	" << TD_USPORA3_SET << " {post-mail}: specify the access nodes set" << endl <<
			endl;

	return lSs.str();
}

set<QueryType> TdUspora3::fSQueryTypes() {
...
#define UNKNOWN ERR("TdUspora3::fProcessArgs: Unknown command." << endl << fGetHelp()); \
	return;
#define MOVEARG lPos++; \
	if (lPos > cArgs.size() - 1) { \
		ERR("TdUspora3::fProcessArgs: Insufficient number of arguments when trying to process arg. number " \
				<< lPos + 1 << endl << fGetHelp()); \
		return; \
	} \
	lCurArg = cArgs[lPos];
void TdUspora3::fProcessArgs() {
	int lPos = -1;
	string lCurArg = "";
	...
	DEB(LVL_CONN, "TdUspora3::fProcessArgs: Arguments processed" << endl);
}
