...
TdUsporaSeg::TdUsporaSeg(vector<string> iArguments) {
	cPreprocessed = false;
...
Ret TdUsporaSeg::fPreprocess(Object *iObject) {
	cGraph = dynamic_cast<TdGraph*>(iObject);

	//----- preprocessing -----
	INFO("Starting preprocessing of USP-OR-A-SEG oracle (for TD graphs)" << endl);
	...
	cFAnUsps = fTdSegUSP(cGraph, lAnVec, lAnVec, DAY_MIN, &cFAnUspInd);

	INFO("Analysing USPs among ANs" << endl);
	INFO(fAnalyzeSegUsps(cUgGraph, cFAnUsps, cFAnUspInd, lAnVec, lAnVec, false));

	if (lRet == OK) {gAlgTimer->fStopTimer();}
	INFO("Ending preprocessing of USP-OR-A-SEG oracle (for TD graphs)" << endl);
...
int TdUsporaSeg::fGetSpace() {
	...
	lBytes += fSizeOf((*cFAnUspInd));
	for (size_t i = 0; i < (*cFAnUspInd).size(); i++) {
		lBytes += fSizeOf((*cFAnUspInd)[i]);
		for (size_t j = 0; j < (*cFAnUspInd)[i].size(); j++) {
			lBytes += fSizeOf((*cFAnUspInd)[i][j]);
			for (size_t k = 0; k < (*cFAnUspInd)[i][j].size(); k++) {
				lBytes += fSizeOf((*cFAnUspInd)[i][j][k]);
			}
		}
	}
...
Connection TdUsporaSeg::fAnUspConn(int iFrom, int iTime, int iTo) {
	if (iFrom == iTo) {
		Connection lConn;
	...
	return fConnUsps(iTime, (*cFAnUsps)[lAnFrom][lAnTo], (*cFAnUspInd)[lAnFrom][lAnTo][lSeg], cGraph);
}
...
#define TERMINATE if (lRet == OK) {gAlgTimer->fStopTimer();} \
		if (lLocalConn.fIsEmpty()) { \
			...
			INFO("Ending querying USP-OR-A-SEG oracle (for TD graphs) for the connections" << endl); \
			return lLocalConn; \
		}

Connection TdUsporaSeg::fQueryConn(string iFrom, int iTime, string iTo) {
	if (cPreprocessed == false) {
		ERR("TdUsporaSeg::fQueryConn: Preprocessing not carried out" << endl);
		return Connection();
	}

	//----- compute -----
	INFO("Starting querying USP-OR-A-SEG oracle (for TD graphs) for a connection" << endl);
	Ret lRet = gAlgTimer->fStartNewTimer();
...
//	}
	...
	if (lRet == OK) {gAlgTimer->fStopTimer();}
	INFO("Ending querying USP-OR-A-SEG oracle (for TD graphs) for the connection" << endl);

	return lFinalConn;
}

int TdUsporaSeg::fQueryEa(string iFrom, int iTime, string iTo) {
	INFO("Starting querying USP-OR-A-SEG oracle (for TD graphs) for the earliest arrival" << endl);
	Ret lRet = gAlgTimer->fStartNewTimer();

	Connection lConn = fQueryConn(iFrom, iTime, iTo);

	if (lRet == OK) {gAlgTimer->fStopTimer();}
	INFO("Ending querying USP-OR-A-SEG oracle (for TD graphs) for the earliest arrival" << endl);

	return lConn.fGetLength();
}

string TdUsporaSeg::fSGetHelp() {
	stringstream lSs (stringstream::out);

	lSs << "HELP USP-OR-A-SEG oracle (for time graphs)" << endl <<
			"	" << TD_USPORA_SEG_SET << " {post-mail}: specify the access nodes set" << endl <<
			endl;

	return lSs.str();
}

set<QueryType> TdUsporaSeg::fSQueryTypes() {
...
#define UNKNOWN ERR("TdUsporaSeg::fProcessArgs: Unknown command." << endl << fGetHelp()); \
	return;
#define MOVEARG lPos++; \
	if (lPos > cArgs.size() - 1) { \
		ERR("TdUsporaSeg::fProcessArgs: Insufficient number of arguments when trying to process arg. number " \
				<< lPos + 1 << endl << fGetHelp()); \
		return; \
	} \
	lCurArg = cArgs[lPos];
void TdUsporaSeg::fProcessArgs() {
	int lPos = -1;
	string lCurArg = "";
	...
	DEB(LVL_CONN, "TdUsporaSeg::fProcessArgs: Arguments processed" << endl);
}
